...
@onready var Llanta: StaticBody3D = self
@onready var OutLineMesh: MeshInstance3D = $old_tyre/MeshInstance3D
...
@export var prompt_offset_y: float = 2.0            # cuánto sube el rótulo
...
func _process(_delta: float) -> void:
	...
	if Llanta:
		Llanta.position.y = (OutLineWidth if selected else 0.0)
