...
namespace PedaleandoGame.Entities.Player
{
	public partial class Player : CharacterBody3D
	{
		...
		// Agua / Submarino
		// Nota: exportar Node directamente en C# no siempre serializa bien; usamos NodePath y resolvemos en _Ready
		[Export] public NodePath WaterAreaPath { get; set; }
		[Export] public NodePath WaterSurfacePath { get; set; }
		...
		public override void _Ready()
		{
			...
			if (WaterAreaPath != null && !WaterAreaPath.IsEmpty)
			{
				_waterArea = GetNodeOrNull<Area3D>(WaterAreaPath);
				if (_waterArea != null)
				{
					_waterArea.BodyEntered += OnWaterBodyEntered;
					_waterArea.BodyExited += OnWaterBodyExited;
				}
			}

			if (WaterSurfacePath != null && !WaterSurfacePath.IsEmpty)
			{
		...
		private void SwimPhysics(double delta)
		{
			...
			// Keep at surface unless the player is actively diving (Control/Crouch)
			if (_waterSurface != null && !Input.IsActionPressed("Crouch"))
			{
				var surfaceY = _waterSurface.GlobalTransform.Origin.Y - SurfaceMargin;
				// If we're below the surface margin and not diving, snap back to the surface and cancel downward velocity
				if (GlobalTransform.Origin.Y < surfaceY)
				{
					GlobalTransform = new Transform3D(
						GlobalTransform.Basis,
						new Vector3(GlobalTransform.Origin.X, surfaceY, GlobalTransform.Origin.Z)
					);
					if (Velocity.Y < 0)
					{
		...
		{
			if (_waterSurface != null && _camera != null)
			{
				bool under = _camera.GlobalTransform.Origin.Y < _waterSurface.GlobalTransform.Origin.Y - SurfaceMargin;
